...
const cases = [
  ...
  { id:'personal', type:'PERSONAL MEDIA OPS', category:'content', year:'PERSONAL PRACTICE', title:'个人自媒体运营', summary:'在小红书与抖音等平台持续做内容实验，独立完成选题、拍摄、图文、剪辑与发布复盘。', result:'32 万阶段性曝光', accent:'#ff879f', cover:'/assets/运营/从0到1运营团队.webp', role:'选题 / 制作 / 平台适配 / 复盘', proof:'2500+ 获赞 · 280+ 粉丝 · 30+ 旅行图文', metrics:[['32 万','抖音阶段性总曝光'],['2500+','阶段性获赞'],['30+','旅行与日常图文']], journey:['定内容方向','独立制作','平台适配','看数据反馈'], gallery:[['/assets/运营/从0到1运营团队.webp','抖音账号与阶段性数据','账号运营'],['/assets/运营/小红书图文1.webp','小红书图文样本 / 选题与正文','图文内容'],['/assets/运营/小红书图文2.webp','小红书视觉样本 / 封面与画面','视觉内容']], detailProof:['年轻用户兴趣 · 旅行与日常 · 多种内容风格','图文策划 · 拍摄 · 文案 · 视觉处理 · 视频剪辑','32 万阶段性曝光 · 2500+ 获赞 · 280+ 粉丝','个人内容实践，不等同于商业增长结果'], details:[['内容方向','围绕年轻用户兴趣、旅行与日常生活持续寻找选题，尝试不同内容风格与表达方式。'],['制作流程','独立完成图文策划、素材整理、拍摄、文案、视觉处理与视频剪辑，并根据平台语境调整封面、标题和节奏。'],['已确认结果','2 个月阶段性总曝光 32 万，获赞 2500+，沉淀粉丝 280+。个人账号数据与实习账号数据分开呈现。'],['运营边界','当前材料主要证明多平台内容生产和账号运营实践，不直接等同于商业增长或收入结果。']] },
...
function openCase(key){const item=cases.find(x=>x.id===key);if(!item)return;document.querySelector('#modal-body').innerHTML=`<span class="modal-kicker">${item.type} / ${item.year}</span><h2>${item.title}</h2><p class="modal-lede">${item.summary}</p><div class="modal-facts"><div><span>ROLE</span><b>${item.role}</b></div><div><span>RESULT</span><b>${item.result}</b></div><div><span>PROOF</span><b>${item.proof}</b></div></div><div class="modal-journey"><span>OPERATING JOURNEY</span><div>${item.journey.map((step,index)=>`<b><i>0${index+1}</i>${step}</b>`).join('')}</div></div><div class="modal-gallery"><div class="modal-gallery-head"><span>SELECTED EVIDENCE</span><b>${item.gallery.length} VISUALS</b></div><div class="modal-gallery-grid">${item.gallery.map(([src,alt,caption])=>`<figure><img src="${src}" alt="${alt}" loading="lazy"><figcaption><b>${caption}</b><span>${alt}</span></figcaption></figure>`).join('')}</div></div><div class="modal-detail-list">${item.details.map(([title,body],index)=>`<section><div class="modal-section-index">0${index+1}</div><div><h3>${title}</h3><p>${body}</p></div></section>`).join('')}</div><div class="modal-boundary">证据边界：数字与职责按当前作品集材料呈现；尚未确认的业务结果已明确标注。</div>`;const modal=document.querySelector('.case-modal');modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');lens.stop()}
